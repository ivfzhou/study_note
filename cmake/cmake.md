...
1. **add_library**(*library_name* [ SHARED | STATIC | MODULE ] *source1* [ *source2* ... ])：创建库。
...
1. **include_directories**([ AFTER | BEFORE ] [ SYSTEM ] *dir1* [ *dir2* ... ])：向编译器添加包含目录。AFTER 表示将目录添加到已有的包含目录之后，BEFORE 表示将目录添加到已有的包含目录之前。默认情况下，目录会被添加到已有的包含目录之后。SYSTEM 是可选的参数，用于指定添加的目录是系统级别的目录。添加头文件。
